...
/*
 * 修订记录：
 * zyiyuan 10:49:55 AM 创建
 */
package com.ruoyi.zxydk.flows.repertory.chat;
...
public class ZxydkFlowChatElementCreator implements ApplicationContextAware, InitializingBean, Serializable{
	...
	// 日志
	private static final Logger logger = LoggerFactory.getLogger(ZxydkFlowChatElementCreator.class);
	 ...
	 * 
	 * @param clz [Class<T extends ZxydkFlowBaseAction>]
	 * 
	 * @return T [<T extends ZxydkFlowBaseAction>]
	 */
	public < T extends ZxydkFlowBaseAction > T newInstance(String className){
		...
		try{
			...
			if( o instanceof ZxydkFlowBaseAction){
	 ...
	 * @return T [<T extends ZxydkFlowBaseAction>]
	 */
	public <T extends ZxydkFlowBaseAction> T createInstance( Class<T> clz ){
		try {
	 ...
	 *  @return T [刷新后的元素实例]
	 */
	public <T extends ZxydkFlowBaseAction> T reflush( T t ) {
		try {
